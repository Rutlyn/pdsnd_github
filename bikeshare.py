...
def choice(prompt, choices=('y', 'n')):
    ...
    while True:
        ...
        prompt = ("\nSomething is not right. Kindly take a look at the format and "
                  "be sure to enter the correct option:\n>")

    return choice


def get_filters():
    """Ask the user to specify the actual city(ies) and filters, month(s) and weekday(s).
    ...
    while True:
        ...
        month = choice("\nFrom January to June, for what month(s) do you "
                       ...
                       months)
        day = choice("\nFor what weekday(s) do you wish to filter bikeshare "
...
def trip_duration_stats(df):
    ...
    total_t_time = (str(int(total_travel_time//86400)) +
                         'd ' +
                         str(int((total_t_time % 86400)//3600)) +
                         'h ' +
                         str(int(((total_t_time % 86400) % 3600)//60)) +
                         'm ' +
                         str(int(((total_t_time % 86400) % 3600) % 60)) +
                         's')
    print('For the selected filters, the total t time is : ' +
          total_t_time + '.')

    # display mean travel time
    mean_t_time = df['Trip Duration'].mean()
    mean_t_time = (str(int(mean_t_time//60)) + 'm ' +
                        str(int(mean_t_time % 60)) + 's')
    print("For the selected filters, the mean t time is : " +
          mean_t_time + ".")
...
def user_stats(df, city):
    ...
    print("Distribute for user types:")
    print(user_types)

    # Display counts of gender
    try:
        gender_distribution = df['Gender'].value_counts().to_string()
        print("\nDistribute for each gender:")
        print(gender_distribution)
    except KeyError:
        print("We apologise! There are no data of user genders for {}."
              .format(city.title()))

    # Display earliest, most recent, and most common year of birth
    try:
        earliest_birth_year = str(int(df['birth year'].min()))
        print("\nFor filtering, the oldest person to ride one "
              "bike was born in: " + earliest_birth_year)
        most_recent_birth_year = str(int(df['Birth Year'].max()))
        print("For filtering, the youngest person to ride one "
              "bike was born in: " + most_recent_birth_year)
        most_common_birth_year = str(int(df['Birth Year'].mode()[0]))
        print("For filtering, the most common birth year among the "
              "riders is: " + most_common_birth_year)
    except:
        print("We apologise! There is no data of birth year for {}."
              .format(city.title()))
...
def raw_data(df, mark_place):
    ...
    print("\nYou chose to view some raw data.")
    ...
    # sort data by column
    if mark_place == 0:
        sort_df = choice("\nHow would you like to filter the way the data is "
                         "displayed in the dataframe? Select Enter to view "
                         "unsorted.\n \n [st] Start Time\n [et] End Time\n "
                         "[td] Trip Duration\n [ss] Start Station\n "
                         "[es] End Station\n\n>",
                         ('st', 'et', 'td', 'ss', 'es', ''))

        asc_or_desc = choice("\nWould you like it to be filtered ascending or "
                             "descending order? \n [a] Ascending\n [d] Descending"
                             "\n\n>",
                             ('a', 'd'))
...
def main():
    while True:
        ...
        mark_place = 0
        while True:
            select_data = choice("\nPlease, kindly select the information you require"
                                 " to derive.\n\n [ts] Time Stats\n [ss] "
